...
export async function POST(request: Request) {
  ...
  const { data: recording } = await supabase
    .from("recordings")
    .select("id, storage_url, status, transcription_id")
    .eq("id", recordingId)
    .maybeSingle();
  ...
  if (signError || !signed?.signedUrl) {
    return NextResponse.json(
      { error: signError?.message ?? "Could not prepare the recording for transcription." },
      { status: 500 }
    );
  }

  try {
    const transcript = await submitTranscription(signed.signedUrl);
